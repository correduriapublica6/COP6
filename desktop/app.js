(() => {
  ...
  const activityIcon = (kind) => kind === "creation" ? "＋" : kind === "status" ? "↔" : kind === "request" ? "▤" : "✎";
  const relativeTime = (value) => { const time = new Date(value || 0).getTime(); if (!time) return "ahora"; const minutes = Math.max(0, Math.floor((Date.now() - time) / 60000)); return minutes < 1 ? "ahora" : minutes < 60 ? `hace ${minutes} min` : minutes < 1440 ? `hace ${Math.floor(minutes / 60)} h` : `hace ${Math.floor(minutes / 1440)} d`; };
  function buildActivity() {
    const items = [];
    records.forEach((item) => { const created = item.creadoEn || item.fechaAvaluo; items.push({ id:`record-${item.id}-create`, kind:"creation", category:"avaluos", text:`Avalúo ${item.numeroAvaluo || "s/n"} creado`, user:item.creadoPorNombre || item.usuario || "Usuario", at:created }); if (item.actualizadoEn && item.actualizadoEn !== created) items.push({ id:`record-${item.id}-edit`, kind:"edit", category:"avaluos", text:`Avalúo ${item.numeroAvaluo || "s/n"} editado`, user:item.actualizadoPorNombre || item.usuario || "Usuario", at:item.actualizadoEn }); });
    technicalAppraisals.forEach((item) => { const created = item.creadoEn || item.fechaAvaluo; items.push({ id:`appraisal-${item.id}-create`, kind:"creation", category:"avaluos", text:`Avalúo ${item.numeroAvaluo || item.numero || "s/n"} creado`, user:item.creadoPorNombre || item.creadoPor || "Usuario", at:created }); if (item.actualizadoEn && item.actualizadoEn !== created) items.push({ id:`appraisal-${item.id}-edit`, kind:"edit", category:"avaluos", text:`Avalúo ${item.numeroAvaluo || item.numero || "s/n"} editado`, user:item.actualizadoPorNombre || item.actualizadoPor || "Usuario", at:item.actualizadoEn }); });
    (window.__controlSolicitudes || []).forEach((item) => { const created=item.creadoEn || item.fecha; items.push({ id:`request-${item.id}-create`, kind:"request", category:"solicitudes", text:`Solicitud ${item.folio || item.id} creada`, user:item.solicitante?.nombre || "Cliente", at:created }); if (item.estado && item.actualizadoEn && item.actualizadoEn !== created) items.push({ id:`request-${item.id}-status`, kind:"status", category:"estados", text:`Solicitud ${item.folio || item.id} editada (Estado: ${item.estado})`, user:item.actualizadoPor || item.asesor || "Usuario", at:item.actualizadoEn }); });
    return items.filter((item) => item.at).sort((a,b)=>new Date(b.at)-new Date(a.at)).slice(0,30);
  }
  function renderRecentActivity(filter="todos") { const list=$("#recentActivityList"); if (!list) return; const items=buildActivity().filter(item=>filter==="todos" || item.category===filter || (filter==="avaluos" && item.user===activeSessionUser)); list.innerHTML=items.length ? items.slice(0,12).map(item=>`<article class="activity-item"><span class="activity-icon activity-${item.kind}">${activityIcon(item.kind)}</span><div><strong>${escapeHtml(item.text)}</strong><small>${escapeHtml(item.user)} · ${escapeHtml(relativeTime(item.at))}</small></div><span class="activity-badge activity-badge-${item.kind}">${item.kind === "creation" ? "Creación" : item.kind === "status" ? "Estado" : item.kind === "request" ? "Solicitud" : "Edición"}</span></article>`).join("") : `<p class="activity-empty">No hay actividad para este filtro.</p>`; }
  function searchGlobal(query="") { const palette=$("#globalSearchPalette"), results=$("#globalSearchResults"); if (!palette || !results) return; const q=String(query).trim().toLocaleLowerCase("es-MX"); palette.hidden=false; if (!q) { results.innerHTML='<p>Busca folios, clientes, ubicaciones o acciones como “crear avalúo”.</p>'; return; } const found=[]; const actions=[{terms:["crear","nuevo","avaluo","avalúo","expediente"],type:"Acción",title:"Crear nuevo avalúo",detail:"Abrir Gestión de Avalúos y comenzar un expediente",action:()=>{switchAppView("avaluos"); window.dispatchEvent(new CustomEvent("control-avaluos:section",{detail:"crear"})); window.dispatchEvent(new CustomEvent("control-avaluos:open-technical-appraisal"));}},{terms:["registrar","registro","recepcion","recepción","avaluo"],type:"Acción",title:"Registrar avalúo",detail:"Abrir Recepción para registrar un avalúo",action:()=>{switchAppView("avaluos"); window.dispatchEvent(new CustomEvent("control-avaluos:section",{detail:"recepcion"}));}},{terms:["nueva","solicitud","solicitudes","servicio"],type:"Acción",title:"Nueva solicitud",detail:"Abrir el formulario de solicitudes de servicio",action:()=>{switchAppView("solicitudes"); window.setTimeout(()=>document.querySelector("#newInternalRequestButton")?.click(),0);}},{terms:["gestion","gestión","avaluos","avalúos"],type:"Sección",title:"Gestión de Avalúos",detail:"Consultar y editar expedientes técnicos",action:()=>{switchAppView("avaluos"); window.dispatchEvent(new CustomEvent("control-avaluos:section",{detail:"crear"}));}},{terms:["buscar","solicitud","solicitudes"],type:"Sección",title:"Buscar una solicitud",detail:"Consultar solicitudes registradas",action:()=>switchAppView("solicitudes")}]; actions.forEach(item=>{if(item.terms.some(term=>q.includes(term))) found.push(item);}); records.forEach(item=>{const text=[item.numeroAvaluo,item.solicitante,item.tipoBien,item.observacion].join(" "); if(text.toLocaleLowerCase("es-MX").includes(q)) found.push({type:"Avalúo", title:`Avalúo ${item.numeroAvaluo}`, detail:item.solicitante || item.tipoBien, action:()=>{switchAppView("avaluos");}});}); technicalAppraisals.forEach(item=>{const text=JSON.stringify(item); if(text.toLocaleLowerCase("es-MX").includes(q)) found.push({type:"Avalúo técnico",title:`Avalúo ${item.numeroAvaluo || item.numero || "s/n"}`,detail:item.solicitante || item.bienesAValuar || "Expediente técnico",action:()=>{switchAppView("avaluos"); window.dispatchEvent(new CustomEvent("control-avaluos:section",{detail:"crear"}));}});}); (window.__controlSolicitudes||[]).forEach(item=>{const text=JSON.stringify(item); if(text.toLocaleLowerCase("es-MX").includes(q)) found.push({type:"Solicitud",title:item.folio || "Solicitud",detail:item.solicitante?.nombre || item.tipoAvaluo || "Solicitud de servicio",action:()=>switchAppView("solicitudes")});}); results.innerHTML=found.length ? found.slice(0,10).map((item,index)=>`<button class="search-result" type="button" data-search-result="${index}"><span>${escapeHtml(item.type)}</span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.detail)}</small></button>`).join("") : '<p>No se encontraron coincidencias.</p>'; results.querySelectorAll("[data-search-result]").forEach(button=>button.addEventListener("click",()=>{found[Number(button.dataset.searchResult)]?.action(); window.__setGlobalSearchActive?.(false);})); }
  function renderAlerts(filter="all") { const list=$("#alertsList"), count=$("#alertsCount"); if(!list) return; const alerts=buildActivity().filter(item=>item.kind!=="creation" && (filter==="all" || item.user===activeSessionUser)); if(count) count.textContent=String(alerts.length); list.innerHTML=alerts.length ? alerts.slice(0,10).map(item=>`<article class="alert-item"><span class="activity-icon activity-${item.kind}">${activityIcon(item.kind)}</span><div><strong>${escapeHtml(item.text)}</strong><small>${escapeHtml(item.user)} · ${escapeHtml(relativeTime(item.at))}</small></div></article>`).join("") : '<p>No hay alertas nuevas.</p>'; }
  function bindHomeIntelligence() { const searchInput=$("#globalSearchInput"), palette=$("#globalSearchPalette"), searchWrap=searchInput?.closest(".global-search-wrap"); let searchBackdrop=document.querySelector("#globalSearchBackdrop"); if (palette && searchWrap && palette.parentElement !== searchWrap) searchWrap.appendChild(palette); if (!searchBackdrop) { searchBackdrop=document.createElement("div"); searchBackdrop.id="globalSearchBackdrop"; (document.querySelector("#appShell") || document.body).appendChild(searchBackdrop); } const setSearchActive=(active)=>{ palette.hidden=!active; searchBackdrop.hidden=!active; document.body.classList.toggle("global-search-active", active); }; window.__setGlobalSearchActive=setSearchActive; searchInput?.addEventListener("input", event=>{ const value=event.target.value.trim(); if (value) { searchGlobal(value); setSearchActive(true); } else setSearchActive(false); }); searchInput?.addEventListener("focus", ()=>{ if (searchInput.value.trim()) setSearchActive(true); }); searchInput?.addEventListener("keydown", event=>{ if (event.key === "Escape") { searchInput.value=""; setSearchActive(false); searchInput.blur(); } }); searchBackdrop.addEventListener("click", ()=>{ setSearchActive(false); }); $("#alertsButton")?.addEventListener("click",()=>{const panel=$("#alertsPanel"), button=$("#alertsButton"); panel.hidden=!panel.hidden; button.setAttribute("aria-expanded",String(!panel.hidden)); if(!panel.hidden) renderAlerts();}); $("#markAlertsReadButton")?.addEventListener("click",()=>{const count=$("#alertsCount"); if(count) count.textContent="0"; $("#alertsList").innerHTML='<p>Todo está al día.</p>';}); $$("[data-activity-filter]").forEach(button=>button.addEventListener("click",()=>{$$("[data-activity-filter]").forEach(item=>item.classList.toggle("is-active",item===button)); renderRecentActivity(button.dataset.activityFilter);})); $$("[data-alert-filter]").forEach(button=>button.addEventListener("click",()=>{$$("[data-alert-filter]").forEach(item=>item.classList.toggle("is-active",item===button)); renderAlerts(button.dataset.alertFilter);})); document.addEventListener("keydown",event=>{if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==="k"){event.preventDefault(); $("#globalSearchInput")?.focus();}}); window.addEventListener("control-avaluos:technical-appraisals-updated",()=>{renderRecentActivity();renderAlerts();}); window.addEventListener("control-avaluos:request-updated",()=>{renderRecentActivity();renderAlerts();}); }
  ...
  window.addEventListener("control-avaluos:subsection-change", (event) => {
    ...
    $("#pageTitle").textContent = details.title;
    $("#pageDescription").textContent = details.description;
    $("#pageTitle").closest(".workspace-header")?.classList.toggle("technical-creation-header", inAvaluos && section === "crear");
