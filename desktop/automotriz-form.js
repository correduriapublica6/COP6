(() => {
  ...
  function factorControls(factors, group) {
    return factors.map((factor) => `<label>${escapeHtml(factor)}<select class="factor-select" data-factor-group="${group}" data-factor-name="${escapeHtml(factor)}">${depreciationOptions.map((option) => `<option value="${option.value}"${option.value === 1 ? " selected" : ""}>${option.label}</option>`).join("")}</select></label>`).join("");
  }
  ...
  function renderTechnicalList() {
    ...
    technicalList.innerHTML = `<div class="operational-table-wrap"><table class="operational-table operational-table-compact"><thead><tr><th>Número de avalúo</th><th>Solicitante</th><th>Tipo</th><th>Bienes</th><th>Fecha</th><th>Vehículos</th><th>Elaboró</th><th>Acciones</th></tr></thead><tbody>${appraisals.length ? appraisals.map((appraisal) => { const isMobiliario = appraisal.tipo === "mobiliario"; const label = isMobiliario ? "Mobiliario y Bienes Diversos" : "Automotriz"; const status = appraisal.estadoExpediente === "borrador" ? " · Borrador" : ""; const count = isMobiliario ? Number(appraisal.bienes?.length || 0) : Number(appraisal.vehiculos?.length || 0); return `<tr><td><strong class="avaluo-list-number">${escapeHtml(appraisal.numeroAvaluo)}</strong></td><td>${escapeHtml(appraisal.solicitante)}</td><td>${label}<small class="technical-record-status">${status}</small></td><td>${escapeHtml(appraisal.bienesAValuar)}</td><td>${escapeHtml(formatDate(appraisal.fechaAvaluo))}</td><td>${count}</td><td>${escapeHtml(appraisal.creadoPor || "Sin registro")}</td><td><div class="technical-row-actions"><button class="text-action-button" data-edit-appraisal="${escapeHtml(appraisal.id)}" type="button">Editar</button><button class="text-action-button" data-preview-appraisal="${escapeHtml(appraisal.id)}" type="button">Ver PDF</button><button class="text-action-button" data-word-appraisal="${escapeHtml(appraisal.id)}" type="button">Ver / Descargar Word</button><button class="text-action-button is-danger" data-delete-appraisal="${escapeHtml(appraisal.id)}" type="button">Eliminar</button></div></td></tr>`; }).join("") : `<tr><td colspan="8" class="empty">No hay expedientes técnicos para esta búsqueda.</td></tr>`}</tbody></table></div>`;
