(() => {
  ...
  function previewTechnicalPdf(appraisal) {
    ...
    reportWindow.addEventListener("load", () => window.setTimeout(() => reportWindow.print(), 120), { once: true });
  ...
  function addVehicle(initialVehicle = null, initialValues = null) {
    ...
    if (initialVehicle) {
      ["nombreUnidad", "marca", "modelo", "anio", "numeroSerie", "placas", "condiciones"].forEach((field) => {
  ...
  function collectVehicle(card) {
    ...
    return {
      nombreUnidad: get("nombreUnidad"), marca: get("marca"), modelo: get("modelo"), anio: get("anio"), numeroSerie: get("numeroSerie"), placas: get("placas"), condiciones: get("condiciones"),
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      message.textContent = `Avalúo automotriz ${folioInput.value} ${editingTechnicalId ? "actualizado" : "guardado"}.`;
